...
pub fn zip_folder(unzipped_source_folder: &PathBuf, zipped_destination_file: &PathBuf) -> Result<(), AppError> {
    ...
    // Zips all of the folder. Check source folder exists, destination zip file can be created if necessary .
...
pub fn zip_mdr_folder(source: SourceDetails, parent_unzipped_src_fdr: &PathBuf, parent_zipped_dest_fdr: &PathBuf) -> Result<usize, AppError> {
    ...
    if source.local_files_grouped {
        zip_mdr_files_in_multiple_folders(&srce_folder, &dest_folder)?;
    }
    else {
...
fn zip_mdr_files_in_single_folder(database_name: &String, srce_folder: &PathBuf, dest_folder: &PathBuf) -> Result<usize, AppError> {
    ...
    if paths.len() > 0 {
        ...
        zip_files(paths, dest_folder, &file_name_stem)
    }
    else {
        Ok(0)
    }
...
fn zip_mdr_files_in_multiple_folders(_srce_folder: &PathBuf, _dest_folder: &PathBuf) -> Result<usize, AppError> {
...
fn zip_files(paths: Vec<PathBuf>, zip_folder_path: &PathBuf, file_name_stem: &String) -> Result<usize, AppError> {
    ...
    for i in 0..(zip_files_needed - 1) {
        ...
        let zip_file_name = format!("{} {} to {}.zip", file_name_stem, start_file, end_file);
        let zip_file_path: PathBuf = [zip_folder_path, &PathBuf::from(zip_file_name)].iter().collect(); 
        let zip_file = File::create(&zip_file_path)?;
        let mut zip = ZipWriter::new(zip_file);

        for j in 0..(files_per_zip - 1) {
            
            // collect these files and zip them to the designated zip archive

            let file_path = &paths [i + j];
            let file = File::open(file_path)?;
            let file_name = file_path.file_name().unwrap().to_str().unwrap();

            // Adding the file to the ZIP archive.
            zip.start_file(file_name, options)
                 .map_err(|e| AppError::ZipError(e, file_path.to_owned()))?;
